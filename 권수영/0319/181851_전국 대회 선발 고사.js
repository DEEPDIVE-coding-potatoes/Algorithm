...
function solution(rank, attendance) {
  const isAttend = rank.filter((_, i) => attendance[i]);
  isAttend.sort((a, b) => a - b);
  const student = isAttend.map(ele => rank.findIndex(r => r === ele));
  return student[0] * 10000 + student[1] * 100 + student[2];
}
