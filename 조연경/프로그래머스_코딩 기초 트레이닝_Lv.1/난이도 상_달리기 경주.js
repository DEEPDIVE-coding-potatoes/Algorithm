...
// 검색하고 풀었음! -> 해시맵(객체)를 활용한 풀이!
function solution(players, callings) {
  const rank = {}; // 선수이름 : 현재 등수 저장할 객체
  ...
  for (let name of callings) {
      let curIdx = rank[name]; // 현재 선수의 등수
      if (curIdx === 0) continue; // 1등은 추월 불가능

      let frontName = players[curIdx - 1]; // 앞 선수의 이름
