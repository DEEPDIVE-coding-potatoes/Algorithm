...
function solution(today, terms, privacies) {
  ...
  let currentDate = (+year.slice(2) * 12 * 28) + (+month * 28) + +day;
  const termObjs = terms.reduce((obj, item) => {
  ...
  privacies.forEach((privacy, i) => {
    ...
    let privacyDate = (+year.slice(2) * 12 * 28) + (+month * 28) + +day;
    let expirationDate = privacyDate + termObjs[type] * 28;
    if (expirationDate <= currentDate) answer.push(i + 1);
  })
