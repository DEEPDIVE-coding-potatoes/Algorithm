...
function solution(video_len, pos, op_start, op_end, commands) {
  ...
  if (getSeconds(op_start) <= curSeconds && curSeconds < getSeconds(op_end)) curSeconds = getSeconds(op_end);
  commands.forEach(command => {
    if (command === "next") {
      curSeconds += 10;
      if (curSeconds > getSeconds(video_len)) curSeconds = getSeconds(video_len);
      if (getSeconds(op_start) <= curSeconds && curSeconds < getSeconds(op_end)) curSeconds = getSeconds(op_end);
    }
    if (command === "prev") {
      curSeconds -= 10;
  ...
  return `${String(parseInt(curSeconds / 60)).padStart(2, '0')}:${String(curSeconds % 60).padStart(2, '0')}`
}