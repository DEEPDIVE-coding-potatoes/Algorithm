...
function solution(id_list, report, k) {
  ...
  report.forEach(str => {
    let [reportId, reportedId] = str.split(' ');
    if (!reportList[reportId].includes(reportedId)) {
      reportList[reportId].push(reportedId);
      reportedCount[reportedId]++;
    }
  })
  let suspendUser = Object.keys(reportedCount).filter(user => reportedCount[user] >= k);
  return id_list.map(user => {
    let count = 0;
    reportList[user].forEach(reportedUser => {
      if (suspendUser.includes(reportedUser)) count++;
    });
    return count;
  });
}
