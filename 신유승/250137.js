...
function solution(bandage, health, attacks) {
  const [castingTime, healPerSec, addHeal] = bandage;
  const MAXHP = health;
  attacks.forEach(([attackTime, damage], i) => {
    if (i === 0) health -= damage;
    else {
      if (health > 0) {
        let passedTime = attackTime - attacks[i - 1][0] - 1;
        health += passedTime * healPerSec + parseInt(passedTime / castingTime) * addHeal;
